...
CREATE TABLE duty_leaves (
  ...
  leave_type VARCHAR(50) DEFAULT 'ordinary', -- ordinary, special
  reason TEXT,
  status VARCHAR(50) DEFAULT 'pending', -- pending, coordinator_approved, faculty_approved, rejected
  pdf_url TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE certificates (
  ...
  certificate_type VARCHAR(50) DEFAULT 'participation', -- participation, achievement
  certificate_url TEXT NOT NULL,
  issued_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Volunteer Metrics
CREATE TABLE volunteer_metrics (
  id SERIAL PRIMARY KEY,
  user_id INTEGER UNIQUE NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  tasks_completed INTEGER DEFAULT 0,
  volunteer_hours NUMERIC(5, 2) DEFAULT 0.00,
  events_managed INTEGER DEFAULT 0,
  contribution_score VARCHAR(10) DEFAULT 'C', -- A+, A, B, C
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_duty_leaves_user_id ON duty_leaves(user_id);
CREATE INDEX idx_duty_leaves_status ON duty_leaves(status);
